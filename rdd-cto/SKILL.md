...
- **A — flow 启动**：如果由 `$r = git rev-parse --show-toplevel; & "$r\rdd-engine\scripts\rdd-flow.cmd" -Command start -Role CTO` 进入，优先使用输出的 prompt / handoff packet，只读取 handoff 列出的需求文档
- **B — 用户指定**：提供 `requirement.md` 路径或口述需求 → 直接读取
- **C — 应用层指针消息**：收到 `请处理 .rdd/changes/archive/<name>/ 下的需求` → 识别为应用层交接，运行 `$r = git rev-parse --show-toplevel; & "$r\rdd-engine\scripts\rdd-flow.cmd" -Command handoff -Role CTO -Archive "<path>"` 拉取交接包
...
- 需要理解现有代码时，按上方「rdd-engine 能力：代码探索」硬规则执行（先 `$r = git rev-parse --show-toplevel; & "$r\rdd-engine\scripts\explore.cmd" -Type explore` 缓存判定，命中零子代理，未命中派 `rdd-explore`）
