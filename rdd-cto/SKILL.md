...
**文件白名单**：仅写入 `.rdd/changes/archive/.../design/` 下的技术方向文档和同目录 `task.md` 的 CTO 列状态更新。不在白名单则拒绝。
...
- **A — 用户指定**：提供 `requirement.md` 路径或口述需求 → 直接读取
- **B — 自动查找**：用户未提供 → 扫描 `.rdd/changes/archive/`，找最新归档，读取 `task.md`。向用户确认找到的需求
