...
description: >
  ...
  到 .rdd/exploration/artifacts/、调用 explore.cmd -Type register 注册缓存、
...
判断无匹配缓存。你的任务是**探索代码、打 tags、写出摘要 + 完整记录、注册到缓存**，
让后续同主题探索能命中。
...
5. **注册缓存**：写完配对产物后，**必须**调用 explore.cmd 注册（走 CLI 才能传 -Tags）：
...
- 代码搜索用 grep/glob/read 工具，bash 仅用于调 `explore.cmd`。
