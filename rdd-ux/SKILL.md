...
| 优先级 | 触发 | 动作 |
|--------|------|------|
| A | flow 启动（`$r = git rev-parse --show-toplevel; & "$r\rdd-engine\scripts\rdd-flow.cmd" -Command start -Role UX`） | 使用输出的 prompt / handoff packet，只读 handoff 列出的需求文档 |
| B | 用户指定需求文件路径或口述需求 | 直接读取/记录 |
| C | 应用层指针消息（形如 `请处理 .rdd/changes/archive/<name>/ 下的需求`） | 识别为应用层交接，运行 `$r = git rev-parse --show-toplevel; & "$r\rdd-engine\scripts\rdd-flow.cmd" -Command handoff -Role UX -Archive "<path>"` 拉取交接包 |
