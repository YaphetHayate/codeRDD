...
```powershell
rdd-engine/rdd-flow.cmd -Command next
```

生成目标角色启动引导（全部任务）：

```powershell
rdd-engine/rdd-flow.cmd -Command start -Role CTO
```

生成目标角色启动引导（单需求，"一需求一会话"）：

```powershell
rdd-engine/rdd-flow.cmd -Command start -Role DEV -TaskIndex 0
rdd-engine/rdd-flow.cmd -Command start -Role DEV -TaskIndex 1
```
...
```powershell
rdd-engine/rdd-flow.cmd -Command handoff -Role DEV -Format markdown
```

写入文件：

```powershell
rdd-engine/rdd-flow.cmd -Command handoff -Role DEV -OutFile ".rdd/handoff/dev.json"
```

指定具体归档路径（精确匹配）：

```powershell
rdd-engine/rdd-flow.cmd -Command next -Archive ".rdd/changes/archive/2026-06-04-engine-adapter-modularization"
rdd-engine/rdd-flow.cmd -Command start -Role CTO -Archive ".rdd/changes/archive/2026-06-04-engine-adapter-modularization"
rdd-engine/rdd-flow.cmd -Command handoff -Role DEV -Archive ".rdd/changes/archive/2026-06-04-engine-adapter-modularization"
```
...
```powershell
explore.cmd -Type explore -Query "分析..." 
```
...
```powershell
rdd-engine/rdd-flow.cmd -Command validate -Role DEV -Archive ".rdd/changes/archive/2026-06-04-engine-adapter-modularization"
```
...
```powershell
rdd-engine/rdd-flow.cmd -Command start -Role <推荐角色> -Format markdown
```

如需并行拉起多个会话（多个同角色需求），每个需求独立启动：

```powershell
rdd-engine/rdd-flow.cmd -Command start -Role DEV -TaskIndex 0 -Format markdown
rdd-engine/rdd-flow.cmd -Command start -Role DEV -TaskIndex 1 -Format markdown
```
