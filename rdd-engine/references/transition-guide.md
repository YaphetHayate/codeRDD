...
```powershell
rdd-engine/scripts/rdd-flow.cmd -Command next -Format markdown
```
...
```powershell
rdd-engine/scripts/rdd-flow.cmd -Command start -Role <目标角色> -OutFile ".rdd/handoff/<role>.md"
```
...
2. 输入 `/rdd-<角色>` —— 命令自动加载角色 SKILL，并通过 `rdd-engine/scripts/rdd-flow.cmd -Command handoff` 拉取最新交接包
...
```powershell
rdd-engine/scripts/rdd-flow.cmd -Command handoff -Role <self> -Archive ".rdd/changes/archive/<archive-name>"
```
