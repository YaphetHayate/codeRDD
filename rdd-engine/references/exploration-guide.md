...
| **探索 worker** | `rdd-explore` 子代理（可写） | 收到 miss dispatch prompt 后：探索代码、按模板写 artifact、调用 `explore.cmd -Type register` 注册、返回摘要 | 写 artifact + 调 register |
...
```powershell
rdd-engine/explore.cmd -Type explore -Query "分析认证模块的中间件链和 Token 刷新机制"
```
...
```powershell
explore.cmd -Type register -Key "认证中间件链和 Token 刷新" `
...
2. 调用 `explore.cmd -Type register` 注册（传入实际分析过的文件列表）
3. 返回一句话摘要给调用方角色
